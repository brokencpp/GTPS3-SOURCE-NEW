...
inline World* WorldDB::get_pointer(const string name) const
{
...
{
public:
	World get(string name) const;
	World* get_pointer(string name) const;
	static AWorld get2(string name);
	WorldDB();
};

vector <ItemDB> itemDefs;
	inline ItemDB getItemDef(const int id) 
	{
		try {
...
void SendCmd(ENetPeer* peer, string cmd, bool c_ = false) {
	...
	else if (actual_command.substr(0, 6) == "/find " && pInfo(peer)->dev == 1) {
		string finding = cmd.substr(6, cmd.length() - 6).c_str();
		string list = "";
		for (int i = 0; i < 11869; i++) {
			string iname = items.at(i).name;
			if (iname.find(finding) != string::npos && i % 2 != 1) {
				list += iname + ": " + to_string(i) + "\n";
			}
			gamepacket_t p;
			p.Insert("OnConsoleMessage");
			p.Insert(list);
	...
	else if (actual_command.substr(0, 5) == "/buy ") 
	{
		if (pInfo(peer)->havegrowid) {
			string itemfind = cmd.substr(5, cmd.length() - 5 - 1);
			if (itemfind.length() < 3) {
				gamepacket_t p;
				p.Insert("OnTalkBubble");
				p.Insert("`wThis item name is too short!");
				p.CreatePacket(peer);
				return;	
			}
		SKIPFindss:;
		...
		string itemLower2;
		vector<ItemDB> itemDefsfind;
		for (char c : itemfind) if (c < 0x20 || c > 0x7A) goto SKIPFindss;
		if (itemfind.length() < 3) goto SKIPFindss;
		for (const ItemDB& item : itemDefs) {
			string itemLower;
	...
	SKIPFinds3s:;
		...
		for (const ItemDB& item : itemDefsfind) 
		{
			if (item.name != "") 
			{
		...
		if (itemfind.length() < 3)
		{
			gamepacket_t p(0, pInfo(peer)->netID);
			p.Insert("OnTalkBubble");
			p.Insert("`wWords should be more than 3!``");
			p.CreatePacket(peer);
		}
		else if (itemDefsfind.size() == 0) 
		{
			gamepacket_t p(0, pInfo(peer)->netID);
			p.Insert("OnTalkBubble");
			p.Insert("`wNo items were found with that name!``");
			p.CreatePacket(peer);
		}
		else {
